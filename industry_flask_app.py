...
from authentication import init_auth, admin_required, student_required, auth_manager
from model_training import PlacementPredictor
# from deep_learning_model import DeepPlacementPredictor  # Moved to try/except block
...
from resume_scorer import ResumeAnalysisEngine
from mock_interview_simulator import MockInterviewEngine
...
resume_scorer = ResumeAnalysisEngine()
interview_simulator = MockInterviewEngine()
...
def generate_assessment_recommendations(ml_features, probability):
    """Generate specific assessment recommendations based on profile analysis"""
    recommendations = []
    
    # Analyze current profile weaknesses
    cgpa = ml_features.get('cgpa', 0)
    projects = ml_features.get('num_projects', 0)
    internships = ml_features.get('num_internships', 0)
    leetcode = ml_features.get('leetcode_score', 0)
    communication = ml_features.get('communication_score', 0)
    languages = ml_features.get('programming_languages', '')
    
    # CGPA and academic performance
    if cgpa < 7.0:
        recommendations.append({
            'area': 'Academic Performance',
            'issue': f'CGPA ({cgpa}) below industry standards',
            'impact': 'High',
            'assessments': [
                {'name': 'Comprehensive Aptitude Test', 'url': '/comprehensive-assessment', 'icon': 'fas fa-brain'},
                {'name': 'Subject Knowledge Test', 'url': '/skill-assessment', 'icon': 'fas fa-book'}
            ]
        })
    ...
    if projects < 2:
        recommendations.append({
            ...
            'issue': f'Only {projects} project(s) - need at least 3',
            'impact': 'High',
            'assessments': [
                {'name': 'Programming Skills Test', 'url': '/skill-assessment', 'icon': 'fas fa-code'},
                {'name': 'Live Coding Challenge', 'url': '/skill-verification', 'icon': 'fas fa-laptop-code'}
            ]
        })
    ...
    if leetcode < 1000:
        recommendations.append({
            'area': 'Coding Proficiency',
            'issue': f'LeetCode score ({leetcode}) below expectations',
            'impact': 'Medium',
            'assessments': [
                {'name': 'Algorithmic Thinking Test', 'url': '/comprehensive-assessment', 'icon': 'fas fa-brain'},
                {'name': 'Programming Language Quiz', 'url': '/skill-assessment', 'icon': 'fas fa-code'}
            ]
        })
    ...
    if communication < 7.0:
        recommendations.append({
            ...
            'issue': f'Communication score ({communication}) needs improvement',
            'impact': 'Medium',
            'assessments': [
                {'name': 'Written Communication Test', 'url': '/communication-assessment', 'icon': 'fas fa-edit'},
                {'name': 'Mock Interview Practice', 'url': '/mock-interview', 'icon': 'fas fa-microphone'}
            ]
        })
    
    # Programming languages
    lang_count = len([l.strip() for l in languages.split(',') if l.strip()]) if languages else 0
    if lang_count < 2:
        recommendations.append({
            'area': 'Programming Languages',
            'issue': f'Limited language proficiency ({lang_count} language(s))',
            'impact': 'Medium',
            'assessments': [
                {'name': 'Multi-Language Assessment', 'url': '/skill-assessment', 'icon': 'fas fa-code'},
                {'name': 'Skill Verification', 'url': '/skill-verification', 'icon': 'fas fa-certificate'}
            ]
        })
    
    # Situational judgment for low probabilities
    if probability < 0.5:
        recommendations.append({
            'area': 'Professional Readiness',
            'issue': 'Overall placement readiness needs improvement',
            'impact': 'Critical',
            'assessments': [
                {'name': 'Situational Judgment Test', 'url': '/situational-judgment-test', 'icon': 'fas fa-users'},
                {'name': 'Resume Analysis', 'url': '/resume-scorer', 'icon': 'fas fa-file-alt'}
            ]
        })
...
def predict_placement():
    ...
    try:
        ...
        if result:
            ...
            # Get comprehensive recommendations with assessment links
            recommendations = []
            improvement_areas = []
            assessment_recommendations = []
            
            if result['probability'] < 0.8:  # Enhanced threshold for recommendations
                analysis = recommendation_engine.analyze_student_profile(ml_features)
                recommendations = analysis.get('recommendations', [])[:5]
                improvement_areas = analysis.get('critical_gaps', [])
                
                # Add specific assessment recommendations
                assessment_recommendations = generate_assessment_recommendations(ml_features, result['probability'])
            ...
            return jsonify({
                ...
                'assessment_recommendations': assessment_recommendations if result['probability'] < 0.8 else [],
                'feature_importance': predictor.get_feature_importance_for_prediction(ml_features) if not USE_DEEP_MODEL else {}
            })
        else:
...
@app.route('/verify-programming-language', methods=['POST'])
@login_required
@student_required
def verify_programming_language():
    """Quick verification quiz for programming languages added to profile"""
    try:
        data = request.get_json()
        language = data.get('language', '').lower()
        
        if language not in ['python', 'java', 'javascript', 'c++']:
            return jsonify({'success': False, 'message': 'Language not supported for verification'})
        
        # Get 5 random questions for quick verification
        verification_questions = skill_engine.get_verification_questions(language, count=5)
        
        return jsonify({
            'success': True,
            'language': language,
            'questions': verification_questions,
            'session_id': f"verify_{current_user.id}_{language}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        })
        
    except Exception as e:
        return jsonify({'success': False, 'message': f'Verification error: {str(e)}'})

@app.route('/submit-language-verification', methods=['POST'])
@login_required
@student_required
def submit_language_verification():
    """Submit programming language verification responses"""
    try:
        data = request.get_json()
        ...
        # Evaluate responses
        score = skill_engine.evaluate_verification_responses(language, responses)
        verification_level = 'verified' if score >= 60 else 'basic'
        ...
        db_manager.save_language_verification(verification_data)
        
        return jsonify({
            'success': True,
            'score': score,
            'verification_level': verification_level,
            'message': f'Language verification complete! Score: {score}%'
        })
        
    except Exception as e:
        return jsonify({'success': False, 'message': f'Verification submission error: {str(e)}'})
...
def student_profiles():
    ...
    try:
        print("Debug: Accessing student profiles page")
        students = db_manager.get_all_students()
        print(f"Debug: Retrieved {len(students)} students for admin view")
        if students:
            print(f"Debug: First student: {students[0] if students else 'None'}")
        return render_template('admin/student_profiles.html', students=students)
    except Exception as e:
        print(f"Error in student profiles route: {e}")
        return render_template('admin/student_profiles.html', students=[])
...
def student_profile_detail(student_id):
    ...
    profile = db_manager.get_student_profile(student_id)
    analytics = db_manager.get_student_analytics(student_id)
    assessments = db_manager.get_student_assessments(student_id)
    predictions = db_manager.get_student_predictions(student_id)
    return render_template('admin/student_profile_detail.html', 
                         profile=profile, analytics=analytics, 
                         assessments=assessments, predictions=predictions)
...
def debug_students():
    ...
    try:
        with db_manager.get_connection() as conn:
            ...
            cursor.execute("SELECT id, first_name, last_name, email, user_type FROM users LIMIT 5")
...
@app.route('/create-sample-student')
@login_required
@admin_required
def create_sample_student():
    ...
    try:
        ...
        result = db_manager.create_user(**sample_data)
        ...
        if result.get('success'):
            ...
            db_manager.update_student_profile(user_id, profile_data)
            
            return jsonify({
                'success': True,
                'message': 'Sample student created successfully',
                'user_id': user_id
            })
        else:
            return jsonify({
                'success': False,
                'message': result.get('message', 'Failed to create student')
            })
...
def performance_analytics():
    ...
    # Define default structure that matches template expectations
    default_analytics = {
        'assessments': {
            'total_assessments': 0, 
            'average_score': 0, 
    ...
    try:
        ...
        user_id = int(current_user.id)
        analytics_data = db_manager.get_student_analytics(user_id)
        
        # Debug output
        print(f"Analytics data structure: {analytics_data}")
        print(f"Analytics data type: {type(analytics_data)}")
        
        return render_template('student/performance_analytics.html', analytics=analytics_data)
        
    except Exception as e:
        print(f"Error getting performance analytics: {e}")
        # Return the same default structure that the working database method returns
        return render_template('student/performance_analytics.html', analytics=default_analytics)
...
# ===== MNC COMPANY ASSESSMENTS =====

@app.route('/mnc-assessment/<company>')
@login_required
@student_required
def mnc_assessment(company):
    """MNC company-specific assessment"""
    return render_template('student/mnc_assessment.html', company=company)

@app.route('/previous-year-questions')
@login_required
@student_required
def previous_year_questions():
    """Previous year questions bank"""
    return render_template('student/previous_year_questions.html')

@app.route('/company-mock-interviews')
@login_required
@student_required
def company_mock_interviews():
    """Company-specific mock interviews"""
    return render_template('student/company_mock_interviews.html')

@app.route('/assessment-reports')
@login_required
@student_required
def assessment_reports():
    """Assessment reports and analytics"""
    return render_template('student/assessment_reports.html')

@app.route('/start-mnc-assessment', methods=['POST'])
@login_required
@student_required
def start_mnc_assessment():
    """Start MNC company assessment"""
    try:
        data = request.get_json()
        ...
        company_configs = {
            'capgemini': {
                'sections': [
                    ...
                    {'name': 'Technical MCQs', 'questions': 30, 'time': 30}
                ],
                'total_time': 75,
            ...
            'ltimindtree': {
                'sections': [
                    {'name': 'Programming Questions', 'questions': 2, 'time': 45},
                    {'name': 'System Design', 'questions': 5, 'time': 15},
                    {'name': 'Technical Aptitude', 'questions': 30, 'time': 30}
                ],
                'total_time': 90,
                'cutoff_percentage': 65
            },
            'tcs': {
                'sections': [
                    {'name': 'Numerical Ability', 'questions': 26, 'time': 26},
                    {'name': 'Verbal Ability', 'questions': 24, 'time': 24},
                    {'name': 'Programming Logic', 'questions': 30, 'time': 30},
                    {'name': 'Advanced Coding', 'questions': 2, 'time': 40}
                ],
                'total_time': 120,
                'cutoff_percentage': 55
            },
            'infosys': {
                'sections': [
                    {'name': 'Logical Reasoning', 'questions': 35, 'time': 35},
                    {'name': 'Verbal Ability', 'questions': 35, 'time': 35},
                    {'name': 'Programming', 'questions': 2, 'time': 45}
                ],
                'total_time': 115,
                'cutoff_percentage': 65
            },
            'wipro': {
                'sections': [
                    {'name': 'English Comprehension', 'questions': 20, 'time': 20},
                    {'name': 'Analytical Ability', 'questions': 20, 'time': 20},
                    {'name': 'Technical Assessment', 'questions': 40, 'time': 40}
                ],
                'total_time': 80,
                'cutoff_percentage': 60
            },
            'accenture': {
                'sections': [
                    {'name': 'Cognitive Ability', 'questions': 90, 'time': 90},
                    {'name': 'Technical Assessment', 'questions': 40, 'time': 40},
                    {'name': 'Communication Assessment', 'questions': 15, 'time': 25}
                ],
                'total_time': 155,
        ...
        config = company_configs.get(company.lower(), company_configs['capgemini'])
        config['company'] = company
        config['session_id'] = f"mnc_{company}_{current_user.id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        return jsonify({
            'success': True,
            'config': config,
            'message': f'Starting {company.title()} assessment'
        })
        
    except Exception as e:
        return jsonify({'success': False, 'message': f'Error starting assessment: {str(e)}'})

@app.route('/submit-mnc-assessment', methods=['POST'])
@login_required
@student_required
def submit_mnc_assessment():
    """Submit MNC assessment responses"""
    try:
        data = request.get_json()
        company = data.get('company')
        ...
        # Calculate score based on company-specific criteria
        total_score = 0
        section_scores = {}
        ...
        for section in config['sections']:
            ...
            # Mock scoring logic (in real implementation, this would evaluate actual answers)
            if section_name in ['Programming Questions', 'Advanced Coding', 'Programming']:
                # Coding questions get higher weightage
                section_score = min(95, len(section_responses) * 15 + 60)
            else:
                # MCQ questions
                section_score = min(100, len(section_responses) * 3 + 50)
            
            section_scores[section_name] = section_score
            total_score += section_score
        
        average_score = total_score / len(config['sections']) if config['sections'] else 0
        passed_cutoff = average_score >= config['cutoff_percentage']
        
        # Save assessment result
        assessment_data = {
            'assessment_type': f'mnc_{company}',
            'company': company,
            'config': config,
            'responses': responses,
        ...
        return jsonify({
            ...
            'results': {
                ...
                'section_scores': section_scores,
                'passed_cutoff': passed_cutoff,
                'cutoff_percentage': config['cutoff_percentage'],
                'company': company.title(),
                'feedback': f"{'Congratulations! You passed' if passed_cutoff else 'Keep practicing! You can improve'} the {company.title()} assessment."
            }
        })
        
    except Exception as e:
        return jsonify({'success': False, 'message': f'Error submitting assessment: {str(e)}'})
